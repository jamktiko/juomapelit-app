...
import { Deck2Component } from '../deck2/deck2.component';
...
constructor (public deck2Component : Deck2Component) {};
...
next() {
  if (this.deck2Component.cardCount < 52) {
      ...
      this.deck2Component.cardCount++;
    } else {
      // If the deck is empty, the game is over
      this.deck2Component.isOver = true;
    }
}

// Switches to next card
nextCard() {
  if (this.deck2Component.loading) {
    return}
  if (this.deck2Component.cardCount === 0){
    this.cardFrontside();
    this.addSuitRank();
    this.curRule = this.shuffledCards[0][0]['rule'];
    this.curRuleHeader = this.shuffledCards[0][0]['name'];
